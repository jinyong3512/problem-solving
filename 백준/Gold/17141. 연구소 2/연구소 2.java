...
public class Main {
    ...
    public static void bfs(int[][] arr, Deque<Data> deque) {
        ...
        while (!deque.isEmpty()) {
            ...
            tmpMax = Math.max(tmpMax, tmpData.depth);
            ...
            for (int direction = 0; direction < 4; direction++) {
                ...
                if (newY >= 0 && newY < arr.length && newX >= 0 && newX < arr[0].length) {
                    if ((arr[newY][newX] == 0 || arr[newY][newX] == 2) && !visited[newY][newX]) {
    ...
    public static boolean checkEmpty(int[][] board, boolean[][] visited) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (!visited[i][j] && (board[i][j] == 0 || board[i][j] == 2)) {
                    return true;
                }
